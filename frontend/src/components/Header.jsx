...
const Header = () => {
    ...
    return (
        <header className="bg-green-100 flex justify-between p-5 px-10 pr-20 ">
            ...
            <img src={"./assets/gumsbanner.png"} className={`w-[15%] h-15` } alt="Banner img"/>
            {isLoggedIn &&
                <div className={`flex gap-2 flex-row items-center max-w-[60%]`}>
                    <div className={`flex flex-row `}>
                        <img src={'./assets/Sample_User_Icon.png'} className={`h-10 border rounded-full`} />
                    </div>
                    <h1>{user.fullName}</h1>
                    ...
                    <button
                        ...
                        onClick={()=> {
                        localStorage.removeItem("token"); window.alert("Successfully Logged Out ");
